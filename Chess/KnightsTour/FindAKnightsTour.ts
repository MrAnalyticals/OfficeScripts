function main(workbook: ExcelScript.Workbook): string {
  ...
  while (j != 64) {
    ...
    //filter for 'Err' value
    foundArray = foundArray.filter((obj) => {
      return obj.search('Err') == -1
    })
    SheetKnightsTour.getRange("k7").setValue(foundArray.toString())
  }
  return "Complete Knights Tour Found!"
}

function moveKnightOffBoard(workbook: ExcelScript.Workbook) {
  let SheetKnightsTour = workbook.getWorksheet('KnightsTour')
  let TheBlackKnight = SheetKnightsTour.getShape("BlackKnight");
  TheBlackKnight.setLeft(562.5);
  TheBlackKnight.setTop(44.25);
}
