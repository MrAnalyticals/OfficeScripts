//ShapeClassExample prop only as at 2233 11Aug2024
class CustomShape {
...
function main(workbook: ExcelScript.Workbook, setText: string, setshape: string, ownerName: string) {
...
// Add a new geometric shape to the worksheet
  let shaped = sheet.addGeometricShape(shapetype); 
  ...
  customShape.setTextSize(12);
    ...
    // Set random positions if not aligning shapes
    let setLeft: number = Math.floor(Math.random() * 200) + 1;
    let setTop: number = Math.floor(Math.random() * 200) + 1;
